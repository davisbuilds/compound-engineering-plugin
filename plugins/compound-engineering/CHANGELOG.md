...
- **`/workflows:review` command** — Added `learnings-researcher` agent to the parallel review phase. The review now searches `docs/solutions/` for past issues related to the PR's modules and patterns, surfacing "Known Pattern" findings during synthesis. Closes the compound loop: `/workflows:compound` writes solutions → `/workflows:plan` reads them before building → `/workflows:review` reads them before reviewing.
